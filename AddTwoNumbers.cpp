...
struct ListNode* addTwoNumbers(struct ListNode* l1, struct ListNode* l2) {
	...
	while (l1 != NULL) {			//如果 l1还没有循环完毕
		struct ListNode *p = NULL;
		p = (struct ListNode *)malloc(sizeof(struct ListNode));   //申请一个 ListNode大小的内存

		if (l1->val+carry >= 10) {
		    p->val = (l1->val+carry)%10;           //将 l2加上进位的值赋给 p
		    carry = 1;                             //进位设置为 1
		} else {
		    p->val = (l1->val+carry)%10;           //将 l2加上进位的值赋给 p
		    carry = 0;                             //进位设置为 0
	...
	while (l2 != NULL) {         //如果 l2还没有循环完毕  
		struct ListNode *p = NULL;
		p = (struct ListNode *)malloc(sizeof(struct ListNode));   //申请一个 ListNode大小的内存
		
		if (l2->val+carry >= 10) {
		    p->val = (l2->val+carry)%10;           //将 l2加上进位的值赋给 p
		    carry = 1;                             //进位设置为 1
		} else {
		    p->val = (l2->val+carry)%10;           //将 l2加上进位的值赋给 p
		    carry = 0;                             //进位设置为 0
